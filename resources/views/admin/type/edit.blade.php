...
<div class="card border-0 shadow-sm rounded-4 bg-body p-4 max-w-2xl mx-auto">
   <form action="{{ route('admin.type.update', $type->id) }}" method="POST">
      ...
      <div class="mb-3">
         ...
         <input type="text" name="type_name" id="type_name" class="form-control rounded-3" value="{{ old('type_name', $type->type_name) }}" required>
      </div>
      ...
      <div class="mb-3">
         ...
         <select name="collection_id" id="collection_id" class="form-select rounded-3">
            ...
            @foreach($collections as $collection)
               <option value="{{ $collection->id }}" {{ old('collection_id', $type->collection_id) == $collection->id ? 'selected' : '' }}>{{ $collection->collection_name }}</option>
            @endforeach
         </select>
      </div>
      ...
      <div class="mb-4">
         ...
         <input type="text" name="slug" id="slug" class="form-control rounded-3" value="{{ old('slug', $type->slug) }}" required>
      </div>

      <div class="d-flex gap-2">
         <button type="submit" class="btn btn-custom-1 rounded-pill px-4">Update Type</button>
