   ...
   <form action="{{ route('admin.produk.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row g-3 mb-3">
         <div class="col-md-8">
            ...
            <input type="text" name="nama_produk" id="nama_produk" class="form-control rounded-3" value="{{ old('nama_produk') }}" required>
         </div>
         <div class="col-md-4">
            <label for="sku" class="form-label fw-semibold">SKU Produk</label>
            <input type="text" name="sku" id="sku" class="form-control rounded-3" value="{{ old('sku') }}">
         </div>
      </div>
         ...
         <div class="col-md-4">
            ...
            <select name="collection_id" id="collection_id" class="form-select rounded-3">
               ...
               @foreach($collections as $col)
                  <option value="{{ $col->id }}" {{ old('collection_id') == $col->id ? 'selected' : '' }}>{{ $col->collection_name }}</option>
               @endforeach
            </select>
         </div>
      </div>
      ...
      <div class="row g-3 mb-3">
         <div class="col-md-6">
            ...
            <select name="type_id" id="type_id" class="form-select rounded-3">
               ...
               @foreach($types as $t)
                  <option value="{{ $t->id }}" {{ old('type_id') == $t->id ? 'selected' : '' }}>{{ $t->type_name }}</option>
               @endforeach
            </select>
         </div>
         <div class="col-md-6">
            ...
            <select name="variant_id" id="variant_id" class="form-select rounded-3">
               ...
               @foreach($variants as $v)
                  <option value="{{ $v->id }}" {{ old('variant_id') == $v->id ? 'selected' : '' }}>{{ $v->variant_name }}</option>
               @endforeach
            </select>
         </div>
      </div>
      ...
      <div class="row g-3 mb-3">
         <div class="col-md-4">
            ...
            <input type="number" step="100" name="harga_reguler" id="harga_reguler" class="form-control rounded-3" value="{{ old('harga_reguler') }}">
         </div>
         <div class="col-md-4">
            <label for="tipe_packing" class="form-label fw-semibold">Tipe Packing</label>
            <input type="text" name="tipe_packing" id="tipe_packing" class="form-control rounded-3" value="{{ old('tipe_packing') }}" placeholder="Cans 200g, Pack 250g">
         </div>
         <div class="col-md-4">
            <label for="inner_kemasan" class="form-label fw-semibold">Inner Kemasan</label>
            <input type="text" name="inner_kemasan" id="inner_kemasan" class="form-control rounded-3" value="{{ old('inner_kemasan') }}">
         </div>
      </div>

      <div class="mb-3">
         <label for="gambar_utama_file" class="form-label fw-semibold">Gambar Utama Produk</label>
      ...
      <div class="mb-3">
         ...
         <textarea name="deskripsi_singkat" id="deskripsi_singkat" rows="2" class="form-control rounded-3">{{ old('deskripsi_singkat') }}</textarea>
      </div>

      <div class="mb-3">
         <label for="deskripsi_lengkap" class="form-label fw-semibold">Deskripsi Lengkap</label>
         <textarea name="deskripsi_lengkap" id="deskripsi_lengkap" rows="4" class="form-control rounded-3">{{ old('deskripsi_lengkap') }}</textarea>
      </div>

      <div class="row g-3 mb-3">
         <div class="col-md-4">
            <label for="link_tokopedia" class="form-label fw-semibold">Link Tokopedia</label>
            <input type="url" name="link_tokopedia" id="link_tokopedia" class="form-control rounded-3" value="{{ old('link_tokopedia') }}">
         </div>
         <div class="col-md-4">
            <label for="link_shopee" class="form-label fw-semibold">Link Shopee</label>
            <input type="url" name="link_shopee" id="link_shopee" class="form-control rounded-3" value="{{ old('link_shopee') }}">
         </div>
         <div class="col-md-4">
            <label for="link_lazada" class="form-label fw-semibold">Link Lazada</label>
            <input type="url" name="link_lazada" id="link_lazada" class="form-control rounded-3" value="{{ old('link_lazada') }}">
         </div>
      </div>

      <div class="form-check mb-3">
         <input type="checkbox" name="is_unggulan" value="1" id="is_unggulan" class="form-check-input" {{ old('is_unggulan') ? 'checked' : '' }}>
