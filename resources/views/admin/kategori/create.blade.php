...
<div class="card border-0 shadow-sm rounded-4 bg-body p-4 max-w-2xl mx-auto">
   <form action="{{ route('admin.kategori.store') }}" method="POST">
      ...
      <div class="mb-3">
         ...
         <input type="text" name="nama_kategori" id="nama_kategori" class="form-control rounded-3" value="{{ old('nama_kategori') }}" required>
      </div>
      ...
      <div class="mb-3">
         ...
         <select name="parent_id" id="parent_id" class="form-select rounded-3">
            ...
            @foreach($parents as $parent)
               <option value="{{ $parent->id }}" {{ old('parent_id') == $parent->id ? 'selected' : '' }}>{{ $parent->nama_kategori }}</option>
            @endforeach
         </select>
      </div>
      ...
      <div class="mb-3">
         ...
         <input type="number" name="urutan" id="urutan" class="form-control rounded-3" value="{{ old('urutan', 0) }}">
      </div>
