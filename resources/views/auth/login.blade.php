...
<body x-data="loginDesktopApp()" x-init="initApp()" @keydown.window="handleGlobalHotkeys($event)"
   @mousemove.window="onDrag($event)" @mouseup.window="stopDrag()" :class="theme === 'dark' ? 'dark' : ''"
   ...
   <canvas id="meshCanvas" class="fixed inset-0 pointer-events-none opacity-40 dark:opacity-30"
      style="z-index: 1;"></canvas>

   <!-- Top Desktop OS Workstation Header Bar -->
   <header
      class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white px-[12px] py-[6px] flex items-center justify-end border-b border-slate-700 z-30 font-mono shadow-sm gap-5">
   ...
   <main class="flex-1 flex items-center justify-center p-4 relative z-20 overflow-hidden">
      ...
      <div
         ...
         <!-- 1. WINDOW TITLE BAR -->
         <div
            class="bg-gradient-to-r from-slate-800 via-slate-700 to-indigo-950 text-white px-[14px] py-[8px] flex items-center justify-between border-b border-slate-600 font-mono text-[11px] select-none">
            <!-- Title & Icon -->
         ...
         <div class="p-4 flex-1 grid grid-cols-12 gap-4 overflow-y-auto">
            ...
            <div
               class="col-span-5 bg-gradient-to-br from-slate-200 via-slate-100 to-amber-500/10 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 border border-slate-300 dark:border-slate-700 rounded-[4px] p-5 flex flex-col items-center justify-between text-center shadow-inner">
               <div class="w-full flex flex-col items-center pt-2">
                  <!-- Company Logo (Clean - No Box Framing) -->
                  <div
                     class="w-full flex justify-center items-center py-[10px] select-none cursor-default pointer-events-none">
                     <img
                        :src="theme === 'dark' ? '{{ asset('images/logo-indraco-invert.png') }}' :
                            '{{ asset('images/logo-indraco.png') }}'"
                        alt="PT Indraco Logo"
               ...
               <div
                  class="w-full pt-4 border-t border-slate-300 dark:border-slate-800 text-[10px] font-mono text-slate-500 dark:text-slate-400 space-y-1 text-center">
                  ...
                  <div class="text-[9px] text-amber-600 dark:text-amber-400 font-mono font-semibold"
                     x-text="connectionUrl"></div>
               </div>
            </div>
            ...
            <div class="col-span-7 flex flex-col justify-between">
               ...
               @if (session('info'))
                  <div
                     class="mb-3 p-2.5 rounded bg-blue-500/10 border border-blue-500/30 text-blue-800 dark:text-blue-300 text-xs font-semibold font-mono flex items-center gap-2">
               ...
               <fieldset class="delphi-groupbox p-4 rounded bg-white/80 dark:bg-slate-950/70 shadow-sm relative">
                  <legend
                     class="px-2 font-mono text-xs font-bold text-amber-700 dark:text-amber-400 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded shadow-sm flex items-center gap-1.5">
                  ...
                  <form action="{{ route('login.post') }}" method="POST" id="loginForm"
                     ...
                     <div>
                        <label for="email"
                           class="block font-mono text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                        ...
                        <div class="relative">
                           ...
                           <input type="email" name="email" id="email" x-ref="emailInput"
                              value="{{ old('email', 'admin@indraco.com') }}" required autofocus
                        ...
                        @error('email')
                           <span
                              class="text-rose-600 dark:text-rose-400 text-[11px] font-mono mt-1 block font-semibold">{{ $message }}</span>
                        @enderror
                     </div>
                     ...
                     <div>
                        ...
                        <div class="relative">
                           ...
                           <input :type="showPassword ? 'text' : 'password'" name="password" id="password"
                              x-ref="passwordInput" value="password" required @keydown="checkCapsLock($event)"
                              ...
                              placeholder="••••••••">
                           <button @click="showPassword = !showPassword; playClickSound()" type="button" tabindex="-1"
                        ...
                        <div x-show="capsLock" x-cloak x-transition
                           ...
                           <i data-lucide="alert-triangle"
                              class="w-3.5 h-3.5 text-amber-500 animate-pulse shrink-0"></i>
                     ...
                     <div
                        ...
                        <button type="submit" @click="playClickSound()"
               ...
               <div class="mt-3 pt-3 border-t border-slate-300 dark:border-slate-800">
                  <div class="flex items-center justify-between mb-2">
                     <span
                        class="font-mono text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">
                        <i data-lucide="zap" class="w-3.5 h-3.5 text-amber-500"></i> Akun Demo Cepat
                     </span>
                  </div>

                  <div class="grid grid-cols-3 gap-2">
                     <button @click="fillLogin('admin@indraco.com')" type="button" title="Autofill Super Admin"
                     ...
                     <button @click="fillLogin('gudang@indraco.com')" type="button" title="Autofill PIC Gudang"
                     ...
                     <button @click="fillLogin('fin@indraco.com')" type="button" title="Autofill PIC Keuangan"
